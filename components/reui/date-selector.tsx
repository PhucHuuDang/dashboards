...
export function useDateSelector({
  ...
  const handlePresetSelect = useCallback(
    (presetValue: string) => {
      ...
      if (presetValue === "today" || presetValue === "yesterday") {
        const date =
          presetValue === "today"
            ? today
            : new Date(
        ...
        if (presetMode === "between") {
          setSelectedDate(date);
          setSelectedEndDate(date);
        } else {
          if (presetMode === undefined && filterType === "between") {
            setFilterType("is");
          }
          setSelectedDate(date);
          setSelectedEndDate(undefined);
      ...
      const days = daysMap[presetValue];
      ...
      const canUseRange =
        allowRange && (presetMode === undefined || presetMode === "between");

      if (canUseRange) {
        if (presetMode === undefined) setFilterType("between");
        setSelectedDate(start);
        setSelectedEndDate(today);
        setCalendarMonth(days > 60 ? subMonths(today, 1) : start);
      } else {
        setSelectedDate(start);
        setSelectedEndDate(undefined);
    ...
    [allowRange, presetMode, filterType],
  );
...
function PresetSelector({ activePreset, onPresetSelect }: PresetSelectorProps) {
  return (
    <div className="flex flex-col flex-wrap items-start justify-between gap-1 sm:gap-1.5">
      {Preset.map((preset) => (
        <Button
          key={preset.value}
          variant="ghost"
          size="sm"
          className={cn(
            activePreset === preset.value && "bg-accent text-accent-foreground",
            "py-1 font-normal sm:px-2.5",
          )}
...
function DateSelectorDayPicker({
  ...
  return (
    <div className={cn("flex w-full items-center justify-between ", className)}>
      ...
      <Separator orientation="vertical" className="mx-1 sm:mx-0" />
...
export function DateSelector({
  ...
  return (
    ...
    >
      <div className={cn("w-full space-y-4 sm:w-[570px]", className)}>
