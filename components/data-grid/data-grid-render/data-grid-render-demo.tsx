        ...
        if (table.options.meta?.onDataUpdate) {
          ...
          for (let i = 0; i < count; i++) {
            ...
            switch (col) {
              case "name":
                value = `${
                  NAME_PREFIXES[(rowIndex + cycle) % NAME_PREFIXES.length]
                } ${rowIndex + 1}`;
                break;
              case "email":
                value = `user${rowIndex + 1}@${
                  EMAIL_DOMAINS[(rowIndex + cycle) % EMAIL_DOMAINS.length]
                }`;
                break;
              case "department":
                value =
        ...
        for (let chunk = 0; chunk < Math.ceil(count / chunkSize); chunk++) {
          ...
          const chunkUpdates = buildUpdates(startIdx, endIdx, (row, col) => {
            switch (col) {
              case "name":
                return `${
                  NAME_PREFIXES[(row + cycle) % NAME_PREFIXES.length]
                } ${row + 1}`;
              case "email":
                return `user${row + 1}@${
                  EMAIL_DOMAINS[(row + cycle) % EMAIL_DOMAINS.length]
                }`;
              case "department":
                return (
